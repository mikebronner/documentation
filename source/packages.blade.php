@extends('_layouts.docs')

@push ("styles")
    <style>
        .widget-wrapper {
            position: relative;
            top: 6px;
        }
    </style>
@endpush

@push ("scripts")
    <script async defer src="https://buttons.github.io/buttons.js"></script>
@endpush

@section('body')
<section class="container mx-auto px-6 py-10 md:py-12">

    <h1>Our Open-Source Packages</h1>
    <hr class="block my-8 border-t-0 border-b border-gray-300">
    <p class="md:text-xl lg:text-2xl">
        Our packages are MIT-licensed open-source projects. Their development
        and maintenance requires a considerable amount of resources, especially
        responding to issues and implementing feature requests.
    </p>
    <p class="md:text-xl lg:text-2xl">
        Please consider
        <span class="widget-wrapper">
            <a
                class="github-button"
                href="https://github.com/sponsors/mikebronner"
                data-icon="octicon-heart"
                data-size="large"
                aria-label="Sponsor @mikebronner on GitHub"
            >
                supporting
            </a>
        </span>
        {{-- <a
            class="inline-block rounded border border-mono-300 text-mono-700 px-3 font-normal lg:text-lg"
            href="https://github.com/sponsors/mikebronner"
            target="_blank"
        >
            <svg
                aria-hidden="true"
                focusable="false"
                class="icon inline-block fill-current text-pink-500"
                role="img"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 512 512"
            >
                <path d="M462.3 62.6C407.5 15.9 326 24.3 275.7 76.2L256 96.5l-19.7-20.3C186.1 24.3 104.5 15.9 49.7 62.6c-62.8 53.6-66.1 149.8-9.9 207.9l193.5 199.8c12.5 12.9 32.8 12.9 45.3 0l193.5-199.8c56.3-58.1 53-154.3-9.8-207.9z"></path>
            </svg>
            supporting
        </a> --}}
        their continued development, especially if you use our packages for work
        or deploy them in production. Your continued support means a lot, and
        will to ensure that we can keep on developing open-source software.
        Thank you!
    </p>

    <div class="mt-12 flex -mx-2 -mx-4 flex-wrap">
        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-sign-in-with-apple"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/214492398/0a8d1a80-eceb-11e9-97b9-9513968c6057"
                    alt="Sign in with Apple for Laravel header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Sign in with Apple</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 32 33"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Socialite"
                        >
                            <path d="M14 29.4V16L0 2V0h32v2L18 16v13.4l8 1.6v2H6v-2l8-1.6zm7-21.242a3 3 0 1 0 0-6 3 3 0 0 0 0 6z" fill="#e394ba" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Provide "Sign In With Apple" functionality to your Laravel app.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-socialiter"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/214933023/94f38c80-ee59-11e9-992c-41ebc7eed234"
                    alt="Socialiter for Laravel header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Socialiter</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 32 33"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Socialite"
                        >
                            <path d="M14 29.4V16L0 2V0h32v2L18 16v13.4l8 1.6v2H6v-2l8-1.6zm7-21.242a3 3 0 1 0 0-6 3 3 0 0 0 0 6z" fill="#e394ba" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Manage user persistence and resolution for Socialite providers.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-governor"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/41706753/37d93d00-f1b1-11e9-9f67-067c80849466"
                    alt="Governor for Laravel header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Governor</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Granular role, permissions, and team management.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-model-caching"
                target="_blank"
            >
                <img class="w-full"
                src="https://repository-images.githubusercontent.com/103836049/b0d89480-f1b1-11e9-8e13-a7055f008fe6"
                alt="Model Caching for Laravel header image."
            >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Model Caching</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Model-based caching, providing  up to 10x performance boost.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-caffeine"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/40729869/26446500-f1b2-11e9-9611-6a2e65688de2"
                    alt="Governor for Laravel header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Caffeine</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Prevent time-outs on forms, so users don't have to start over.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-casts"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/54227416/9eab2600-f1b2-11e9-9aa3-2da687ef463c"
                    alt="Casts for Laravel header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Casts</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Expressive form builder using blade directives or aliases.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-messenger"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/22565797/33158880-f1b3-11e9-9950-303089f954dc"
                    alt="Messenger for Laravel header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Messenger</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Notifying your users doesn't have to be a lot of work.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <img
                class="w-full"
                src="https://repository-images.githubusercontent.com/42419266/0f534200-f1b5-11e9-9ca7-57b0e1fe7764"
                alt="Mixpanel for Laravel header image."
            >
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-mixpanel"
                target="_blank"
            >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Mixpanel</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Easy integration of MixPanel analytics, automated user stats.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/nova-multi-tenant-manager"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/186168087/0a8f8d80-f1b7-11e9-93ee-6399860d51f4"
                    alt="Multi-Tenant Manager for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Multi-Tenant Manager</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Manage your tenants and their site settings in multi-tenant apps.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/nova-gutenberg"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/188099261/91913580-f1b8-11e9-8e2e-9a3f6e6da8c2"
                    alt="Gutenberg for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Gutenberg Field</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Add a Gutenberg text editor field to your Nova resources.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/nova-telescope"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/186876699/bfc34500-f1b9-11e9-8ff3-4b4180303dec"
                    alt="Telescope for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Telescope Integration</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        More than just a link, integrate Telescope UI into Nova.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/nova-horizon"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/186876625/7293a300-f1ba-11e9-924f-7785237b1e67"
                    alt="Horizon for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Horizon Integration</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        More than just a link, integrate Horizon UI into Nova.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/nova-map-marker-field"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/193125151/642cd600-9590-11e9-99d0-d77a638bf308"
                    alt="Map Marker Field for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Map Marker Field</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Visual map field for editing latitude and longitude in Nova.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/nova-file-upload-field"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/192123976/6392e900-91f8-11e9-88b5-74365e97af5a"
                    alt="File Upload Field for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>File Upload Field</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        The easiest drag-and-drop file uploading field for Laravel Nova.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/nova-passport-manager"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/203880869/911a9c80-f295-11e9-8b40-9c5a7aefebd2"
                    alt="Passport Manager for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Passport Manager</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 44 60"
                            xmlns="http://www.w3.org/2000/svg" 
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Passport"
                        >
                            <path d="M4.472,60l15,-30l-15,-30l-4.472,0l15,30l-15,30l4.472,0Zm12,0l15,-30l-15,-30l-4.472,0l15,30l-15,30l4.472,0Zm12,0l15,-30l-15,-30l-4.472,0l15,30l-15,30l4.472,0Z" style="fill:rgb(97,178,233);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Manage Passport clients and tokens from within Nova.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-appleseed"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/46007656/88c36100-f297-11e9-8bb9-c0917e799f0f"
                    alt="Appleseed for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Appleseed</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Eliminate favicon error log entries, return 404 non-existant.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-authorization-addons"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/105207145/2dde3980-f298-11e9-8cd8-1eb8c187de10"
                    alt="Authorization Addons for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Authorization Addons</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Helper methods and blade directives for more complex auth.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-changelog"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/189888584/d096b800-f298-11e9-977e-e6e3925132ae"
                    alt="Changelog for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Changelog</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Integrate and display formatted changelog and version info.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-collection-macros"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/134581296/5b2be700-f29a-11e9-91e5-8fab51fa20e4"
                    alt="Collection Macros for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Collection Macros</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Extending Laravel Collections with useful functionality.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-impersonator"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/90768538/6b44c600-f29c-11e9-88af-65e839679460"
                    alt="Impersonator for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Impersonator</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Impersonate users in your Laravel app.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-null-carbon"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/119314805/59692e80-f341-11e9-86d0-068d0824e96d"
                    alt="Null-Carbon for Laravel header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Null-Carbon</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        A Laravel Carbon null-class, better than returning null.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-optimized-postgres"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/94943563/38eea380-f344-11e9-95f0-1cd990116866"
                    alt="Optimized Postgres for Laravel header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Optimized Postgres</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Optimize textual field types when using Postgres databases.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-tawk"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/204191162/32612b80-f346-11e9-8bff-dca042a4ae5d"
                    alt="Tawk for Laravel header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Tawk</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Integrate Tawk.to customer chat into your Laravel apps.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-imagery"
                target="_blank"
            >
                <img
                    class="w-full"
                    src="https://repository-images.githubusercontent.com/93322764/8faaac00-f349-11e9-9b4d-89244bed67ab"
                    alt="Passport Manager for Laravel Nova header image."
                >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Imagery</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Cache and serve responsive images for faster page loads.
                    </p>
                </div>
            </a>
        </div>
    </div>

    <h2 class="mt-8">Forked Packages</h2>
    <p>
        The following packages were originally developed by other developers,
        but either have been abandoned or are not maintained very often.
    </p>

    <div class="flex -mx-2 -mx-4 flex-wrap">
        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-cashier-braintree"
                target="_blank"
            >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Cashier (Braintree)</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Expressive, fluent interface to Braintree's billing services.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-pivot-events"
                target="_blank"
            >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Pivot Events</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Fire model events not provided by Laravel on BelongsToMany.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/nova-prepopulate-searchable"
                target="_blank"
            >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Prepopulate Searchable</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 29 30"
                            xmlns="http://www.w3.org/2000/svg"
                            style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;"
                            title="Nova"
                        >
                            <path d="M25.096,25.569c-2.745,2.737 -6.548,4.431 -10.751,4.431c-4.679,0 -8.863,-2.1 -11.642,-5.399c5.953,4.917 14.82,4.609 20.407,-0.923c3.766,-3.728 3.766,-9.771 0,-13.499c-3.765,-3.728 -9.87,-3.728 -13.635,0c-1.614,1.597 -1.614,4.188 0,5.785c1.614,1.598 4.23,1.598 5.844,0c0.538,-0.532 1.41,-0.532 1.948,0c0.537,0.533 0.537,1.396 0,1.929c-2.69,2.662 -7.051,2.662 -9.74,0c-2.69,-2.663 -2.69,-6.98 0,-9.643c4.841,-4.792 12.69,-4.792 17.531,0c4.829,4.781 4.842,12.524 0.038,17.319l0,0Zm0.89,-20.17c-5.952,-4.917 -14.819,-4.609 -20.407,0.923c-3.766,3.728 -3.766,9.771 0,13.499c3.765,3.728 9.87,3.728 13.636,0c1.613,-1.597 1.613,-4.188 0,-5.785c-1.614,-1.598 -4.231,-1.598 -5.844,0c-0.538,0.532 -1.41,0.532 -1.948,0c-0.538,-0.533 -0.538,-1.396 0,-1.929c2.689,-2.662 7.05,-2.662 9.739,0c2.69,2.663 2.69,6.98 0,9.643c-4.841,4.792 -12.69,4.792 -17.531,0c-4.835,-4.787 -4.841,-12.544 -0.019,-17.338c2.743,-2.725 6.539,-4.412 10.733,-4.412c4.678,0 8.862,2.1 11.641,5.399l0,0Z" style="fill:rgb(37,45,55);"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Prepopulate BelongsTo searchable fields with data.
                    </p>
                </div>
            </a>
        </div>

        <div class="px-3 py-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
            <a
                class="rounded overflow-hidden shadow-lg block"
                href="https://github.com/GeneaLabs/laravel-maps"
                target="_blank"
            >
                <div class="px-6 py-4 text-gray-700 text-base font-normal">
                    <h4>Maps</h4>
                    <p class="my-1">
                        Integrates with:
                        <svg
                            class="icon inline-block"
                            viewBox="0 0 50 52"
                            xmlns="http://www.w3.org/2000/svg"
                            title="Laravel"
                        >
                            <path d="M49.626 11.564a.809.809 0 0 1 .028.209v10.972a.8.8 0 0 1-.402.694l-9.209 5.302V39.25c0 .286-.152.55-.4.694L20.42 51.01c-.044.025-.092.041-.14.058-.018.006-.035.017-.054.022a.805.805 0 0 1-.41 0c-.022-.006-.042-.018-.063-.026-.044-.016-.09-.03-.132-.054L.402 39.944A.801.801 0 0 1 0 39.25V6.334c0-.072.01-.142.028-.21.006-.023.02-.044.028-.067.015-.042.029-.085.051-.124.015-.026.037-.047.055-.071.023-.032.044-.065.071-.093.023-.023.053-.04.079-.06.029-.024.055-.05.088-.069h.001l9.61-5.533a.802.802 0 0 1 .8 0l9.61 5.533h.002c.032.02.059.045.088.068.026.02.055.038.078.06.028.029.048.062.072.094.017.024.04.045.054.071.023.04.036.082.052.124.008.023.022.044.028.068a.809.809 0 0 1 .028.209v20.559l8.008-4.611v-10.51c0-.07.01-.141.028-.208.007-.024.02-.045.028-.068.016-.042.03-.085.052-.124.015-.026.037-.047.054-.071.024-.032.044-.065.072-.093.023-.023.052-.04.078-.06.03-.024.056-.05.088-.069h.001l9.611-5.533a.801.801 0 0 1 .8 0l9.61 5.533c.034.02.06.045.09.068.025.02.054.038.077.06.028.029.048.062.072.094.018.024.04.045.054.071.023.039.036.082.052.124.009.023.022.044.028.068zm-1.574 10.718v-9.124l-3.363 1.936-4.646 2.675v9.124l8.01-4.611zm-9.61 16.505v-9.13l-4.57 2.61-13.05 7.448v9.216l17.62-10.144zM1.602 7.719v31.068L19.22 48.93v-9.214l-9.204-5.209-.003-.002-.004-.002c-.031-.018-.057-.044-.086-.066-.025-.02-.054-.036-.076-.058l-.002-.003c-.026-.025-.044-.056-.066-.084-.02-.027-.044-.05-.06-.078l-.001-.003c-.018-.03-.029-.066-.042-.1-.013-.03-.03-.058-.038-.09v-.001c-.01-.038-.012-.078-.016-.117-.004-.03-.012-.06-.012-.09v-.002-21.481L4.965 9.654 1.602 7.72zm8.81-5.994L2.405 6.334l8.005 4.609 8.006-4.61-8.006-4.608zm4.164 28.764l4.645-2.674V7.719l-3.363 1.936-4.646 2.675v20.096l3.364-1.937zM39.243 7.164l-8.006 4.609 8.006 4.609 8.005-4.61-8.005-4.608zm-.801 10.605l-4.646-2.675-3.363-1.936v9.124l4.645 2.674 3.364 1.937v-9.124zM20.02 38.33l11.743-6.704 5.87-3.35-8-4.606-9.211 5.303-8.395 4.833 7.993 4.524z" fill="#FF2D20" fill-rule="evenodd"/>
                        </svg>
                    </p>
                    <p class="mt-3 mb-0">
                        Integrate Google maps easily to Laravel's views.
                    </p>
                </div>
            </a>
        </div>

    </div>
</section>
@endsection
